...
class Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        listeRegion = []
        for row in result:
            listeRegion.append(row[0])
        
        self.input = ttk.Combobox(self, values=listeRegion)
        ...
        columns = ('code_departement', 'Nb de mesures','moyennesdestempmoyennes')
    ...
    def searchRegion(self, event = None):
        ...
        # Si la saisie contient quelque chose
            ...
            #Changez la requête fournie pour afficher le nombre de mesures prises"
            # " et la moyenne des températures moyennes par département pour la région choisie."
            try:
                cursor = db.data.cursor()
                result = cursor.execute("""SELECT code_departement, COUNT(*) AS nbmesuresprises, 
